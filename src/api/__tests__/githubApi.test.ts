...
describe('fetchUsers', () => {
  it('fetches users successfully from GitHub API', async () => {
    const users = [{ login: 'testuser', id: 1, avatar_url: 'http://avatar.url' }];
    ...
    expect(fetchMock.mock.calls[0][0]).toContain('https://api.github.com/search/users?q=testuser');
  });
...
describe('fetchRepositories', () => {
  it('fetches repositories successfully from GitHub API', async () => {
    const repositories = [{ id: 1, name: 'testrepo', html_url: 'http://repo.url', description: 'Test Repo' }];
    ...
    expect(fetchMock.mock.calls.length).toEqual(1);
    expect(fetchMock.mock.calls[0][0]).toContain('https://api.github.com/users/testuser/repos');
  });
